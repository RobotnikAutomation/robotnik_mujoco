...
def generate_launch_description():	
    ...
    xacro_file = os.path.join(robotnik_mujoco_path,
                              'models/rbwatcher',
                              'rbwatcher.urdf')  # make sure the arg prefix:=robot_ when converting from xacro
    ...
    controller_config_file = os.path.join(robotnik_mujoco_path, 'config', 'rbwatcher_controller_params.yaml')
    ...
    node_mujoco_ros2_control = Node(
        ...
        parameters=[
            ...
            {'mujoco_model_path':os.path.join(robotnik_mujoco_path, 'worlds', 'world_watcher.xml')}
        ]
    )
